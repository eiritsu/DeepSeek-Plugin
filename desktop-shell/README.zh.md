...
- **原生应用生命周期。** Swift 桌面壳会先为 Application Support 数据目录持有 advisory lock，再在操作系统选择的随机 loopback 端口启动仓库构建出的 `dsh web`，等待健康检查通过，并用 WKWebView 嵌入界面，不会额外打开浏览器标签页。启动其他 App 副本时，新进程会激活已有 App，并在针对同一份会话数据启动第二个 runtime 前退出。一体化透明标题栏会在主内容顶部空白处保留原生拖拽区域，同时避开侧边栏控件。
