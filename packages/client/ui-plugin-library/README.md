...
Community discovery remains separate from manual source review and now uses the [SkillHub Plugins catalog](https://skillhub.cloud.tencent.com/plugins) as its single external source. The catalog contributes names, categories, descriptions, stars, and repository links; this metadata is neither an endorsement nor an installability decision. A Review action immediately opens Review & install. Native code pins the listed GitHub repository to a commit before using the same structural and security preflight. An entry that cannot be pinned or does not expose a valid DSH Bundle receives no installation token.

The catalog retains its search term, page cursor, and scroll position. Opening the library prefetches the first SkillHub page. The paginated API is backed by a short native page cache; the catalog viewport loads the next page near its end, and an explicit Load more button preserves keyboard access.
