...
const configuredBinDir = (process.env.LARKSUITE_CLI_BIN_DIR || "").trim();
const binDir = configuredBinDir
  ? path.resolve(configuredBinDir)
  : path.join(__dirname, "..", "bin");
const bin = path.join(binDir, "lark-cli" + ext);
...
if (args[0] === "install") {
  console.error("The embedded launcher does not expose the npm setup wizard; configure Lark in DSH Settings.");
  process.exit(2);
} else {
  // Auto-download binary if missing (e.g. npx skipped postinstall).
  if (!fs.existsSync(bin)) {
    try {
      execFileSync(process.execPath, [path.join(__dirname, "install.cjs")], {
        stdio: "inherit",
      ...
      console.error(
        ...
        `  node "${path.join(__dirname, "install.cjs")}"\n`
      );
      process.exit(1);
    }
  }
